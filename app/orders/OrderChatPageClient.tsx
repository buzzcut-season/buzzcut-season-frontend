...
export function OrderChatPageClient({ params, role }: OrderChatPageClientProps) {
  ...
  const loadInitialChat = useCallback(async (orderId: number) => {
    ...
    try {
      const response =
        role === "buyer" ? await getOrderChatMessages(orderId, { size: 50 }) : await getSellerOrderChatMessages(orderId, { size: 50 });
  ...
  const loadOlderMessages = useCallback(async () => {
    ...
    try {
      const response =
        role === "buyer"
          ? await getOrderChatMessages(resolvedOrderId, {
              size: 50,
              beforeMessageId: chatCursor,
            })
          : await getSellerOrderChatMessages(resolvedOrderId, {
              size: 50,
              beforeMessageId: chatCursor,
            });
      const normalized = [...(response.messages ?? [])].reverse();
  ...
  }, [chatCursor, chatLoadingMore, resolvedOrderId, role]);
  ...
  useEffect(() => {
    const isChatAvailable =
      order?.status === "PAID" || order?.status === "COMPLETED" || order?.status === "REFUNDED";
  ...
  const onSendMessage = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const sendRole = role === "buyer" ? "buyer" : "seller";
        ws.send(
          serializeStompFrame(
            "SEND",
            {
              destination: `/app/ws/order-chat/${resolvedOrderId}/${sendRole}/send`,
        ...
        const latest =
          role === "buyer"
            ? await getOrderChatMessages(resolvedOrderId, { size: 50 })
            : await getSellerOrderChatMessages(resolvedOrderId, { size: 50 });
